                    		...
                    		<c:forEach var="message" items="${messageList}" varStatus="messageNum">
                    			<c:if test="${member.id==message.sendId && message.seAva == 0}"> <!-- 보낸쪽지 -->
                   					<tr align="center">
                    					...
                    					<c:if test="${message.seRead == 0}"> <!-- 보낸쪽지가 읽음표시가 아니라면 -->
                    						<td class="text-right text-info bg-warning" id="messageContent">
                    							<a class="text-info" href="${contextPath}/message/readMessage.do?messageNO=send${message.messageNO}">${message.content}</a>
                    						</td>
                    					</c:if>
                    					<c:if test="${message.seRead == 1}"> <!-- 보낸쪽지가 읽음표시 라면 -->
                    						<td class="text-right text-muted bg-warning" id="messageContent">
                    							<a class="text-muted" href="${contextPath}/message/readMessage.do?messageNO=send${message.messageNO}">${message.content}</a>
                    						</td>
                    					</c:if>
                    			...
                    			<c:if test="${member.id==message.toId && message.reAva == 0}"> <!-- 받은쪽지 -->
                    				<tr align="center">
                    					...
                    					<td style="width:10%">${message.sendId}</td>
                    					<td style="width:10%">${message.toId}</td>
                    					<c:if test="${message.reRead == 0}"> <!-- 받은쪽지가 읽음표시가 아니라면 -->
	                    					<td class="text-left text-info bg-light" id="messageContent">
	                    						<a class="text-info" href="${contextPath}/message/readMessage.do?messageNO=receive${message.messageNO}">${message.content}</a>
	                    					</td>
                    					</c:if>
                    					<c:if test="${message.reRead == 1}"> <!-- 받은쪽지가 읽음표시 라면 -->
                    						<td class="text-left text-muted bg-light" id="messageContent">
	                    						<a class="text-muted" href="${contextPath}/message/readMessage.do?messageNO=receive${message.messageNO}">${message.content}</a>
	                    					</td>
                    					</c:if>
                    					<td style="width:20%">${message.writeDate}</td>
                   					</tr>
